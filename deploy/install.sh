...
print_complete() {
    ...
    echo "  1. 复制客户端配置到本地电脑:"
    echo "     scp root@$SERVER_IP:$INSTALL_DIR/client.yaml.example ~/.claude-forward/client.yaml"
    echo ""
    echo "  2. 复制客户端程序到本地电脑:"
    echo "     scp root@$SERVER_IP:$INSTALL_DIR/client ~/claude-forward-client"
    echo ""
    echo "  3. 在本地电脑启动客户端:"
    echo "     ./claude-forward-client -config ~/.claude-forward/client.yaml"
    echo ""
